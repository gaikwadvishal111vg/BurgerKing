...
orderButton.addEventListener("click",()=>{
   let indexRandom = Math.floor(Math.random()*ticked.length);
    ...
    if(id == "burger"){
        setTimeout(()=>{
            imageContainer.innerHTML = `<img class="w-[450px] h-[300px] mt-10 rounded-xl" src="Burger.jpg" alt="burger">`;
            BoxorderNo.innerText = `Order No : ${orderNumber}`;
        },RandomTime);
    }
    else if(id == "frenchfries"){
        setTimeout(()=>{
            imageContainer.innerHTML = `<img class="w-[450px] h-[300px] mt-10 rounded-xl" src="Fries.jpeg" alt="Fries">`;
            BoxorderNo.innerText = `Order No : ${orderNumber}`;
        },RandomTime);
    }
    else if(id == "colddrink"){
        setTimeout(()=>{
            imageContainer.innerHTML = `<img class="w-[450px] h-[300px] mt-10 rounded-xl" src="ColdDrink.jpeg" alt="Glass">`;
            BoxorderNo.innerText = `Order No : ${orderNumber}`;
        },RandomTime);    
    }
